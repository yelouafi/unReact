...
function sortableList(sItems) {
  ...
  var move = function move(arr, elm, targetIdx) {
    var srcIdx = arr.indexOf(elm);
    if (srcIdx === targetIdx || srcIdx < 0 || targetIdx < 0) return arr;
  ...
  app.tap('dragOver', function (ev) {
    return ev.source.preventDefault();
  });

  var dragSource = (0, _srcApp.step)(null, app.on('dragStart'));

  var yAdjust = function yAdjust(ev) {
    return ev.clientY - ev.target.offsetTop <= ev.target.offsetHeight / 2 ? 0 : 1;
  };

  var dropIndex = (0, _srcApp.step)(null, app.on('dragOver').map(function (item, ev) {
    return item === 'placeholder' ? dropIndex(-1) : items().indexOf(item) + yAdjust(ev);
  })),
      isDragOver = (0, _srcApp.step)(false, app.on('dragOver').map(function (_) {
    return true;
  }), app.on('dragEnd').map(function (_) {
    return false;
  })),
      items = (0, _srcApp.step)(sItems, app.on('dragEnd').map(function (_) {
    return move(items(-1), dragSource(), dropIndex());
  }));
  ...
  app.view = function () {
    ...
    return (0, _snabbdomH2['default'])('ul', fstSection.map(function (it, i) {
